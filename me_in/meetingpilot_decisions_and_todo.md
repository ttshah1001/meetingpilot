...
- **LLM provider: Google Gemini (AI Studio free tier), not Anthropic Claude.** Switched to avoid paying for API calls. `gemini-3.6-flash` supports both forced tool/function calling (needed for the structured JSON extraction schema) and native image input (needed for the screenshot multimodal feature) in one model, so it covers both LLM calls without a second provider. `meetingpilot/llm.py` still wraps Anthropic and needs a rewrite to Gemini's `google-genai` SDK before this is real — not done yet, tracked as a Build TODO below. `ANTHROPIC_API_KEY` / Claude path kept as optional/legacy in config and the access-check script, not removed, in case of a fallback comparison. **Confirmed working end-to-end** via `scripts/check_access.py` on 2026-08-19 — all 4 doors (Gemini text, Gemini vision, Calendar events, Gmail compose) pass. Still untested against the actual class rubric constraints (pending, rubric not yet reviewed).
  ...
  - **Correction (2026-08-20): both `gemini-3.5-flash` and `gemini-3.6-flash` share the same 20-requests/day free-tier cap** on this API key (`quotaId: GenerateRequestsPerDayPerProjectPerModel-FreeTier`, per-model). The earlier note that 3.6-flash "did not hit this limit" was premature — it just hadn't been tested enough yet at the time. We hit it for real on 2026-08-20 after a full day of development testing. **This is a genuine demo-day risk**, not just a dev annoyance: 20 requests/day covers roughly 10 full meeting-processing runs (2 LLM calls each) before the app stops working entirely until the quota resets, with each real Gemini API key getting its own independent 20/day. The retry-with-backoff added earlier does NOT help here — it's a daily quota, not a transient rate limit, so retrying just fails again.
  - **Default model switched to `gemini-flash-lite-latest`** (2026-08-20). Google doesn't publish exact free-tier RPD per model in the docs (checked directly — not disclosed), but Flash-Lite tiers are built for high-volume/low-cost use and are historically the most quota-generous free tier in Gemini's pricing structure; this is a reasoned bet, not a verified "zero limit" claim. Confirmed working for both required capabilities before switching: forced function calling (real extraction against `samples/01_sprint_planning.txt`, 7 items) and multimodal image input (synthetic Kanban screenshot, correctly tagged `source: screenshot`). Full live pipeline re-verified afterward on this model: extraction, planning, Calendar dry-run, Gmail dry-run, and `.ics` export (7 real files written and parsed back with `icalendar` to confirm correctness) all worked end-to-end in one run.
  ...
  - **Real bug found while preparing DEMO.md (2026-08-20): multimodal extraction got crowded out by a busy transcript.** Built `samples/04_kanban_board.png` as a demo asset, tested it paired with `samples/01_sprint_planning.txt` (7 spoken items) — the model dropped all 4 Kanban cards entirely, 0 tagged `source: screenshot`. Isolated the cause: raw vision worked perfectly (asked the model to just describe the image — got a correct, detailed transcription), and the *same* screenshot against a near-empty transcript correctly extracted all 4 cards. So it wasn't a vision failure or a bad image — under the forced-schema extraction call, a transcript with several real items crowded out screenshot content entirely instead of extracting from both. Fixed by strengthening `EXTRACT_SYSTEM` in `extraction.py` to explicitly instruct the model to treat screenshot extraction as a separate, mandatory pass regardless of transcript size. Reverified twice against the exact failing case (busy transcript + Kanban image) — 11/11 items correctly extracted both times (7 transcript + 4 screenshot), correctly excluding the "Done" column card as already-completed. This was a real reliability gap that would have made the multimodal demo beat silently fail on stage.
...
- [x] **.ics export** (done 2026-08-20): new `meetingpilot/ics_export.py` using `icalendar`. No API keys, no network — validated with real round-trip tests (`Calendar.from_ical()` parses generated bytes back and checks fields, not just "didn't crash"). Factored shared description-text logic (`PlannedItem.description_text()`, `.resolved_due_date()`) out of `calendar_tool.py` into `models.py` so Calendar/Gmail/.ics all stay consistent instead of duplicating the same formatting three times. Wired into `cli.py` (`--export-ics DIR`, one file per dated item) and `app.py` (per-item "Download .ics" via `st.download_button`, native, no OAuth popup — plus a "download all as one bundle" button). 5 new tests, 51/51 total passing. **Not verified through a fresh live full-pipeline run** — hit the Gemini free-tier daily quota (20 req/day, see below) right when testing this; `.ics` generation itself doesn't depend on the LLM at all (operates on already-planned items) so this doesn't block confidence in it, but the CLI/UI wiring specifically wasn't re-run against a live meeting today.
