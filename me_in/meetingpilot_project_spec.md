...
- **New (2026-08-19): Gemini free-tier transient errors.** Hit real `503 UNAVAILABLE` (model overloaded) and `429 RESOURCE_EXHAUSTED` (daily quota, only on `gemini-3.5-flash`, not `gemini-3.6-flash`) repeatedly during dev testing — not rare edge cases. Mitigated with retry-with-backoff in `llm.py` (3 attempts, 2s/4s backoff) for both codes. Unlike the Calendar/Gmail fallback (`.ics` export, TODO #3), there is no fallback for a sustained Gemini outage — extraction and planning both require a live LLM call, so a full Google-side outage during the demo would still be a hard failure. Worth rehearsing with, not just hoping around.
...
2. **Gmail draft tool** — add `gmail.compose` scope to the existing OAuth flow (same `credentials.json` pattern as Calendar). Draft-only, never sends. Dry-run mode shows the exact MIME payload, matching the existing Calendar dry-run pattern.
3. **.ics export** — use `icalendar` or `ics` Python package to write one `.ics` file per task. No API keys. Serves as a live-demo fallback if Google OAuth or the venue's wifi fails.
...
|---|---|---|
| "Layer you build around the LLM core" (workflow/tools/memory/planning) | Two-call extraction/planning split + SQLite memory + Calendar/Gmail/.ics tools | ✅ multimodal extraction + Calendar built, 🔲 Gmail/.ics pending |
...
| Demo shows meaningful end-to-end behavior | Upload → multimodal extraction → planning → memory → tool output, live or recorded | ✅ multimodal extraction verified live end-to-end (transcript + screenshot → tagged items → planning → memory); 🔲 Gmail/.ics still pending |
