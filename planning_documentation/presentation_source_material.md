...
- **69 items / 95% avg. extraction confidence** — Total real action items extracted across all 16 `Sample_DATA` meetings (transcript-only, no screenshots — a conservative floor), averaging **4.3 items/meeting** at **0.95 average confidence**. Only 1% fell below the 0.6 low-confidence threshold; only 1% had no stated owner; 9% had no stated due date (correctly left null rather than invented).
  - **Scoring methodology**: confidence is not a post-hoc calculation over separate signals — it's a single float (0-1) the model itself assigns per item in the same forced-schema extraction call, per an explicit rubric in the system prompt (`meetingpilot/extraction.py`): **0.9+** when owner, task, and date are all stated explicitly in the transcript; **0.5-0.7** when something is implied rather than stated outright; **below 0.5** when the commitment itself is shaky/ambiguous. Items below the separately configured `LOW_CONFIDENCE_THRESHOLD` (default `0.6`) get auto-flagged `needs_review` in the UI regardless of what the model's own confidence number says, so a demo audience sees the actual scoring logic, not a black box.
