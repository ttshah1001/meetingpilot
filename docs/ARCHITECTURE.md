...
**Why Gemini, not Anthropic.** The original implementation used Claude; switched to Google Gemini (free tier, AI Studio) to avoid paying for API calls. `gemini-3.6-flash` supports both forced function calling (needed for structured JSON extraction) and native image input (needed for screenshots) in one model. `ANTHROPIC_API_KEY` still exists in config as an optional/legacy path, but the app itself only calls Gemini.
...
- **Gemini free-tier transient errors.** `503 UNAVAILABLE` (overloaded) and `429 RESOURCE_EXHAUSTED` (quota) both occurred repeatedly during development — not rare edge cases. Mitigated with retry-with-backoff (`meetingpilot/llm.py`), including a retry path for a successful-but-truncated response missing the tool call (hit once on a large multi-item planning payload). A sustained Gemini outage during a live demo would still be a hard failure — there is no fallback for the LLM layer itself, unlike Calendar/Gmail (dry-run) or a future `.ics` export.
