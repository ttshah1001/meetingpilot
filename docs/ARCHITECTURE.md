...
**Chat-based refinement (`refine_summary()` in `summary.py`).** After the first pass, a chat box lets the user request changes ("make it shorter," "add a diagram for X") without starting over. The refine call is grounded in the same original transcript/screenshots *and* the current draft (sent back as JSON in the prompt), with an explicit "this is an edit, not a rewrite" instruction — verified live that unrelated content stays materially the same when only one part is asked to change. The original `TranscriptDocument`/`Screenshot` objects are kept in `st.session_state` (not just the derived `PipelineResult`) specifically so refinement has something to stay grounded in. The call offers the model *two* tools (`call_tool_choice()` in `llm.py`) — `submit_summary` for an actual edit, or `reply_in_chat` for anything else — so an off-topic message ("draft the email Raj mentioned") gets an honest plain-text answer instead of the earlier behavior, which forced every message through the summary schema and always claimed "updated" even when nothing relevant changed.

**Every external-write tool has a dry-run mode.** Calendar, Gmail, and Google Tasks all default to dry-run, printing the exact API payload/MIME preview instead of executing. Gmail is additionally *structurally* draft-only — the module only ever calls `drafts().create`, never `send`. The draft's subject/body are LLM-composed (`compose_email()` in `gmail_tool.py`), grounded in the item's task/owner/due date/source quote — not a template dump of internal fields like confidence or priority, which is what a recipient would actually see if this were live.
...
- **Gemini free-tier *daily* quota (`429 RESOURCE_EXHAUSTED`) is a real demo-day risk, not just a transient error.** Both `gemini-3.5-flash` and `gemini-3.6-flash` are capped at 20 requests/day per API key (each real run costs 2+ calls: extraction, planning, optionally the summary/diagram call) — hit for real during development on 2026-08-20. The existing retry logic treats 429 as retryable, which is only correct for short rate-limit bursts; for a genuinely exhausted daily quota, retrying just fails again a few seconds later for no benefit (Gemini's 429 response doesn't distinguish the two cases in a way the client currently checks). **Mitigated (2026-08-20) by switching the default model to `gemini-flash-lite-latest`** — Google doesn't publish exact free-tier RPD per model, but Flash-Lite tiers are built for high-volume use and are historically the most quota-generous free tier; a full extraction+planning+Calendar+Gmail+.ics run was re-verified live on this model with no quota error. Still worth getting a fresh API key close to demo day as a backstop, since "much higher" isn't a documented guarantee of "unlimited." There's also a separate *per-minute* limit (15 requests/min on the free tier) — hit once during a benchmark script that ran 16 extraction calls back-to-back with no delay; spacing calls out avoids it.
- **Fixed: raw LLM prose rendered through Streamlit's markdown parser without escaping.** Hit live on real demo transcripts, not hypothetical — underscore-heavy phrasing (e.g. "mid_market") got read as italic-span delimiters, swallowing the underscores and the spaces between words, and paired dollar amounts (e.g. `$85k...$95k`) got read as inline LaTeX math, both producing visibly garbled text. This affected every site rendering raw LLM/extraction output: the summary, source quotes, merged quotes, planning notes, owner names, due-date text. Fixed with a `_escape_markdown()` helper (`app.py`) applied at every one of those sites. Found in the same pass: the chat bubble renderer interpolated raw chat text directly into an HTML block with `unsafe_allow_html=True`, unescaped — an actual HTML/script injection path, not just a display bug, now `html.escape()`'d.
- **Mermaid diagrams intermittently crash client-side on real transcripts** ("Syntax error in text" from mermaid.js) — the model sometimes generates invalid syntax: unquoted labels containing colons/spaces/parens, or `direction` on the same line as a `subgraph` header. Mitigated (not eliminated, since it's free-form model output) by adding explicit syntax-hygiene rules to the generation and refine system prompts (`meetingpilot/summary.py`): one statement per line, quote any label with punctuation, alphanumeric-only node/subgraph IDs, matching `end` per subgraph.
